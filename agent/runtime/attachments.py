...
DOC_EXTS = {".pdf"}
ATTACH_EXTS = IMAGE_EXTS | DOC_EXTS
DEFAULT_MAX_MB = 10
...
def extract_attachments(line: str) -> tuple[str, list[str]]:
    ...
    are real media files (quoted or bare), peel them out, and return
    ``(prompt_text, [paths])``. Requiring the file to actually exist means a word
    that merely looks like a path won't be misread as an attachment.
    """
    candidates: list[str] = []
    ...
    for cand in candidates:
        ...
        if path.is_file() and path.suffix.lower() in ATTACH_EXTS:
